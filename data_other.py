...
def load_from_directory(source_dir, split, batch_size):
    fs = FilenamesDataset(source_dir)
    fs.split(split)
    ds_train = ImageDataset(fs.train_filenames, fs.train_cls)
    ds_train.load()
    ds_test = ImageDataset(fs.test_filenames, fs.test_cls)
    ds_test.load()
